...
"SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS amount FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING amount >= 0"
end
